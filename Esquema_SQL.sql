...
CREATE OR REPLACE VIEW public.actor_num_peliculas
AS SELECT (a.first_name::text || ' '::text) || a.last_name::text AS nombre_actor,
    count(fa.film_id) AS cantidad_peliculas
   FROM actor a
     LEFT JOIN film_actor fa ON a.actor_id = fa.actor_id
  GROUP BY a.actor_id, a.first_name, a.last_name
  ORDER BY (count(fa.film_id)) DESC, ((a.first_name::text || ' '::text) || a.last_name::text);

-- Permissions

ALTER TABLE public.actor_num_peliculas OWNER TO postgres;
GRANT ALL ON TABLE public.actor_num_peliculas TO postgres;
